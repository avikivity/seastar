...
struct call_then_impl<future<when_all_succeed_tuple<T...>>> {
    ...
    static auto run(future<was_tuple>& fut, Func&& func) noexcept {
        // constructing func in the lambda can throw
        try {
            ...
            return futurize<result_type<Func>>::make_exception_future(std::current_exception());
        }
    }
};
...
public:
    ...
    SEASTAR_CONCEPT( requires ::seastar::CanInvoke<Func, T...> || internal::CanInvokeWhenAllSucceed<Func, T...>)
    Result
