...
run_with_scheduling_group(scheduling_group sg, Func func, Args&&... args) {
    ...
    if (sg.active()) {
        ...
        internal::schedule_in_group(sg, [cur, pr = std::move(pr), func = std::move(func), args = std::make_tuple(std::forward<Args>(args)...)] () mutable {
            return futurator::apply(func, std::move(args)).then_wrapped([cur, pr = std::move(pr)] (future<> fut) mutable {
                internal::schedule_in_group(cur, [fut = std::move(fut), pr = std::move(pr)] () mutable {
