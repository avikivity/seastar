...
struct futurized_action_helper<AsyncAction, false> {
    struct wrapper {
        ...
        futurize_t<orig_ret> operator()() const {
            return futurize<orig_ret>::make_ready_future(action());
        };
    };
    using type = wrapper;
};
...
class do_until_state final : public continuation_base<> {
    ...
    AsyncAction _action;
public:
    explicit do_until_state(do_until_position position, StopCondition stop, AsyncAction action) : _position(position), _stop(std::move(stop)), _action(std::move(action)) {}
    future<> get_future() { return _promise.get_future(); }
    virtual void run_and_dispose() noexcept override {
        ...
        try {
            do {
                if (_position == do_until_position::new_cycle && _stop()) {
                    _promise.set_value();
                    delete this;
                    return;
                }
...
future<> do_until(StopCondition stop_cond, AsyncAction action) {
    ...
    do {
        ...
        if (!f.available()) {
            auto task = std::make_unique<do_until_state<StopCondition, AsyncAction>>(do_until_position::processing_result, std::move(stop_cond), std::move(action));
    ...
    auto task = std::make_unique<do_until_state<StopCondition, AsyncAction>>(do_until_position::new_cycle, std::move(stop_cond), std::move(action));
