...
append_challenged_posix_file_impl::must_run_alone(const op& candidate) const noexcept {
    ...
    return (candidate.type == opcode::truncate) || (candidate.type == opcode::flush && _fsync_is_exclusive);
}
...
private:
    ...
    void mark_run_end() {
        ...
        _run_start = std::chrono::steady_clock::now();
        _run_start_rusage = get_rusage();
        _stalls += _run_start_rusage.ru_nvcsw - start_nvcsw;
...
std::ostream& operator<<(std::ostream& os, const stall_report& sr) {
    ...
    return os << format("{} stalls, {} ms stall time, {} ms run time", sr.stalls, to_ms(sr.stall_time), to_ms(sr.run_wall_time));
}
